...
namespace cp52936
{
    public class GB18030Encoding
    {
        ...
        public GB18030Encoding(bool throwOnInvalidBytes)
        {
                                ...
                                if (n >= 4)
                                {
                                    ...
                                    if (c4 >= 0x30 && c4 <= 0x39)
                                    {
                                        ...
                                        if (i <= 39419)
                                        {
                                            ...
                                            {
                                                uint diff = gb18030uni_ranges[k1];
                                                pwc = (ulong)(i + diff);
                                                return 4;
                                            }
                                        }
        ...
        private static int gb2312_wctomb(ref byte[] r, long wc, int n)
        {
            ...
            if (n >= 2)
            {
                if (wc >= 0x0000 && wc < 0x0460)
                    used = gb2312_uni2indx_page00[idx = (int)(wc >> 4), 1];
                else if (wc >= 0x2000 && wc < 0x2650)
                    used = gb2312_uni2indx_page20[idx = (int)(wc >> 4) - 0x200, 1];
                else if (wc >= 0x3000 && wc < 0x3230)
                    used = gb2312_uni2indx_page30[idx = (int)(wc >> 4) - 0x300, 1];
                else if (wc >= 0x4e00 && wc < 0x9cf0)
                    used = gb2312_uni2indx_page4e[idx = (int)(wc >> 4) - 0x4e0, 1];
                else if (wc >= 0x9e00 && wc < 0x9fb0)
                    used = gb2312_uni2indx_page9e[idx = (int)(wc >> 4) - 0x9e0, 1];
                else if (wc >= 0xff00 && wc < 0xfff0)
                    used = gb2312_uni2indx_pageff[idx = (int)(wc >> 4) - 0xff0, 1];
                if (idx >= 0)
                {
                    int i = (int)(wc & 0x0f);
        ...
        private static readonly ushort[] big5_2charset_1 =
        {
        ...
        private static readonly ushort[] big5_2charset_2 =
        {
        ...
        static int big5_wctomb(ref byte[] r, long wc, int n)
        {
            if (n >= 2)
            {
                ...
                if (wc >= 0x0000 && wc < 0x0100)
                    used = big5_uni2indx_page00[idx = (int)(wc >> 4), 1];
                else if (wc >= 0x0200 && wc < 0x0460)
                    used = big5_uni2indx_page02[idx = (int)((wc >> 4) - 0x020), 1];
                else if (wc >= 0x2000 && wc < 0x22c0)
                    used = big5_uni2indx_page20[idx = (int)((wc >> 4) - 0x200), 1];
                else if (wc >= 0x2400 && wc < 0x2650)
                    used = big5_uni2indx_page24[idx = (int)((wc >> 4) - 0x240), 1];
                else if (wc >= 0x3000 && wc < 0x33e0)
                    used = big5_uni2indx_page30[idx = (int)((wc >> 4) - 0x300), 1];
                else if (wc >= 0x4e00 && wc < 0x9fb0)
                    used = big5_uni2indx_page4e[idx = (int)((wc >> 4) - 0x4e0), 1];
                else if (wc >= 0xfa00 && wc < 0xfa10)
                    used = big5_uni2indx_pagefa[idx = (int)((wc >> 4) - 0xfa0), 1];
                else if (wc >= 0xfe00 && wc < 0xff70)
                    used = big5_uni2indx_pagefe[idx = (int)((wc >> 4) - 0xfe0), 1];
                if (idx >= 0)
                {
                    int i = (int)(wc & 0x0f);
                    if ((used & (uint)((ushort)1 << i)) != 0)
                    {
                        ushort c;
                        /* Keep in `used' only the bits 0..i-1. */
                        used &= ((ushort)1 << i) - 1;
                        /* Add `indx' and the number of bits set in `used'. */
                        used = (used & 0x5555) + ((used & 0xaaaa) >> 1);
                        used = (used & 0x3333) + ((used & 0xcccc) >> 2);
                        used = (used & 0x0f0f) + ((used & 0xf0f0) >> 4);
                        used = (used & 0x00ff) + (used >> 8);
                        if ((idx + used) < big5_2charset_1.Length)
                            c = big5_2charset_1[idx + used];
                        else
                            c = big5_2charset_2[idx + used - big5_2charset_1.Length];
        ...
        {
            return Gb18030_GetString(szIn, szIn.Length);
        }

        public string Gb18030_GetString(byte[] szIn, int len)
        {
            string szOut = "";
            ulong code = 0;
            int i;
        ...
        public static bool isUTF8(string strIn)
        {
            ...
            for (int i = 0; i < len; ++i)
            {
                if ((strIn[i] & 0x80) == 0) // normal ASCII
                    continue;
                int l = 0;
                if ((strIn[i] & 0xE0) == 0xC0) // 2-byte
                    l = 1;
                else if ((strIn[i] & 0xF0) == 0xE0)  // 3-byte
                    l = 2;
                else if ((strIn[i] & 0xF8) == 0xF0) // 4-byte
                    l = 3;
        ...
        public static bool isUTF8(byte[] strIn)
        {
            ...
            for (int i = 0; i < len; ++i)
            {
                ...
                if ((strIn[i] & 0xE0) == 0xC0) // 2-byte
                    ...
                    k = 3;
                if (k == 0 || i + k >= len) // no UTF leader or not enough bytes
                    return false;
